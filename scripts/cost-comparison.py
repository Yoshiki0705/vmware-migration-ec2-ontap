...
def calculate_fsxn_cost(
    ...
    pricing: FsxnPricing = FsxnPricing()
) -> dict:
    """
...
def generate_comparison_report(
    ...
    lines.append("")
    lines.append(f"**リージョン**: ap-northeast-1 (東京)")
    ...
    lines.append("")
    lines.append("> ⚠️ 料金は 2026年6月時点の公開情報に基づく概算。最新料金は AWS 公式ページで確認。")
    lines.append("")
    lines.append("## 構成比較")
    lines.append("")
    lines.append("| 項目 | 構成 A: EBS gp3 のみ | 構成 B: EBS + FSxN | 構成 C: EBS io2 (高IOPS) |")
    lines.append("|------|---------------------|-------------------|------------------------|")
    lines.append(f"| OS ディスク | EBS gp3 50GB | EBS gp3 50GB | EBS gp3 50GB |")
    lines.append(f"| データディスク | EBS gp3 {data_size_gb}GB | FSxN iSCSI {data_size_gb}GB | EBS io2 {data_size_gb}GB |")
    lines.append(f"| IOPS | {iops} | FSxN (NVMe cache) | {iops} |")
    lines.append(f"| Snapshot | EBS Snapshot | ONTAP Snapshot (即時) | EBS Snapshot |")
    lines.append(f"| Clone | 不可 | FlexClone (即時) | 不可 |")
    lines.append(f"| Replication | EBS 間コピー | SnapMirror (効率的) | EBS 間コピー |")
    lines.append(f"| Dedup/Compression | なし | あり ({round((1-efficiency_ratio)*100)}% 削減想定) | なし |")
    lines.append("")
    ...
    lines.append(f"- FSxN 容量プール ({fsxn['capacity_pool_gb']} GB): ${fsxn['cost_capacity_pool']:.2f}")
    ...
    lines.append(f"| 構成 | 月額 | 年額 | 対構成 A 比 |")
    lines.append(f"|------|------|------|-----------|")
    lines.append(f"| A: EBS gp3 のみ | ${total_a:.2f} | ${total_a*12:.2f} | — |")
    lines.append(f"| B: EBS + FSxN | ${total_b:.2f} | ${total_b*12:.2f} | {((total_b/total_a)-1)*100:+.1f}% |")
    lines.append(f"| C: EBS io2 | ${total_c:.2f} | ${total_c*12:.2f} | {((total_c/total_a)-1)*100:+.1f}% |")
    lines.append("")
    ...
    lines.append("> 運用効率（Snapshot/Clone/DR）+ 実効容量削減 + I/O 柔軟性を含めた TCO で判断すべき。")
    lines.append("")
    lines.append("---")
    lines.append("")
...
def main():
    ...
    if args.json:
        results = {
            "fsxn": calculate_fsxn_cost(args.data_size, args.throughput, efficiency_ratio=args.efficiency),
            "ebs_gp3": calculate_ebs_gp3_cost(args.data_size, iops=args.iops, throughput_mbps=args.throughput),
