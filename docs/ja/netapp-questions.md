...
EC2 は AMI（EBS バックド）からのみブート可能で、FSxN iSCSI LUN から直接起動はできない、という物理制約の確認。

| # | 質問 | 優先度 |
|---|------|--------|
| Q1 | Shift Toolkit Early Preview は OS ディスクの AMI 変換まで含むか、データディスクの FSxN 配置のみか? | Critical |
...
| # | 質問 | 優先度 |
|---|------|--------|
| Q5 | AWS Transform の FSxN 移行は内部で Shift Toolkit / FlexClone / SnapMirror を利用するのか、AWS ネイティブのブロックレプリケーションか? | Critical |
...
| Q8 | AWS Transform でコンピュート（ルート = EBS）、Shift Toolkit でデータ（FSxN）を分担する構成は推奨構成として成立するか? | High |

## 3. FSxN 移行先としての仕様

| # | 質問 | 優先度 |
|---|------|--------|
| Q9 | AWS Transform の FSxN 移行先はブロック（iSCSI LUN）のみか、NFS データストア相当も対象か? | High |
...
- AWS Transform の FSxN 宛先 UI: VMware migration の移行ウェーブ計画フロー内で確認（ジョブ作成・discovery データ投入が前提）
