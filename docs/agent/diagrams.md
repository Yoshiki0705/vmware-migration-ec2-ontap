...
**補足ボックスは両図から削除済み。** 載っていた実測値はすべて
`docs/ja|en/atx-fsxn-ga-verification.md` の本文と表にあり、消えたのは事実ではなく二重管理。
`Note` シェイプごと削除したのは、残すことが次の図で文字の壁を画像に入れる誘いになるため。

`atx-fsxn-finalize-flexclone` はそれと**縦積みへの変更**で下限を満たした（925 × 710、
実効 15.6px）。横 4 段では 1480px を要し、その幅の下限 `fontSize` 24 では英語の
"Independent FlexVol after the split" が列に収まらない。縦は幅を争わない。

`atx-fsxn-migration-overview` に残っているのは**寸法の決定**である。5 段の直列と 2 本の
並列分岐を持ち、最長ラベルが 16px で 224px あるため、横 1 列では 880px のカラムに入らない。
取れる形は 2 つで、どちらも記事側に影響する。

| 形 | 結果 | 記事への影響 |
|---|---|---|
| 1 図のまま縦に積む | 760 × 1550、縦横比 1:2 | ファイル名と alt text は不変。長い縦スクロールになる |
| データ経路と管理経路に分割 | どちらも横長のまま収まる | 図が 1 枚増える。alt text・キャプション・本文の参照が変わる |
